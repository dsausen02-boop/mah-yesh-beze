...
והכי חשוב — הוא הרב שלהם, לא כותב טור:
- הוא פונה אליהם ישירות: ״תלמידיי היקרים״, ״ואני יודע שיש בכם מי ש...״, ״תכתבו לי״.
- הוא מזכיר מכתבים ושאלות שקיבל מהם, וזוכר מי נוסע לבד, מי מארח בפעם הראשונה,
  מי צם השנה בפעם הראשונה. הוא קורא הכל, גם כשאינו מספיק לענות.
- פעם במאמר הוא נעצר ואומר משהו כן לגמרי, בלי בדיחה — עידוד, נחמה, או היתר
  להקל על עצמם. מיד אחר כך הוא חוזר להומור, כדי לא להכביד.
- בענייני בריאות, בדידות או לחץ הוא רציני ומיטיב: הבריאות והנפש קודמות לכל
  חומרה, ואין להתבייש.
- הוא מסיים בברכה אישית מירושלים, ומזמין אותם לכתוב אליו שוב.
...
שלב ב׳ — כתוב מאמר ארוך, 1000–1400 מילים בעברית, בקול של הרב עזרא, שמחבר בין \
הנושא של השבוע לבין מה שמצאת. שלב את פרטי המציאות באופן טבעי — לא כרשימה. זהו \
מכתב שבועי לתלמידים, לא טור קצר: יש בו מקום לסיפור, לפרטים קטנים מהבית, ולרגע \
אחד של כנות בלי בדיחה.
...
{{
  ...
  "greeting": "פתיחה אישית של 2–4 משפטים הפונה אל התלמידים",
  "paragraphs": ["פסקה", "פסקה", "..."],
  "letter": {{
    "from": "חתימה קצרה ואנונימית, למשל ׳ר׳ מרמת גן׳",
    "question": "שאלה של תלמיד, 2–4 משפטים, מהחיים האמיתיים",
    "answer": "תשובת הרב, 3–6 משפטים: מעשית, חמה, ובלי לזלזל בשואל"
  }},
  "ruling": {{"text": "פסק קצר בלשון הלכתית מומצאת עם ניקוד חלקי", "source": "שם מקור מומצא"}},
  "israel": ["מה שקורה בישראל השבוע, משפט לכל פריט", "..."],
  "closing": "פסקה קצרה לסיום, 2–3 משפטים",
  "blessing": "ברכה אישית מירושלים, משפט או שניים, ובסופה הזמנה לכתוב אליו",
  "sources": ["כתובת אתר שהסתמכת עליה", "..."]
}}

הנחיות למבנה: 8–12 פסקאות, כל אחת 90–150 מילים. "israel" — 2 עד 3 פריטים קצרים. \
"letter" — שאלה שנשמעת אמיתית (בן אדם שעבר דירה, מארח בפעם הראשונה, נוסע לבד, \
לא בטוח בעצמו), ולא שאלה מצחיקה בלבד. "sources" — הכתובות שמהן לקחת את מה שקורה \
בישראל.{tone_line}
"""
...
def validate(essay):
    ...
    if not isinstance(essay["paragraphs"], list) or len(essay["paragraphs"]) < 6:
        raise ValueError("expected at least 6 paragraphs — this should read as a long letter")
    # Hebrew is compact — 850 words here reads like a long letter, not a column.
    words = sum(len(p.split()) for p in essay["paragraphs"])
    if words < 850:
        raise ValueError(f"too short: {words} words in the body, wanted about 1000")
    for key in ("greeting", "blessing"):
        if not essay.get(key):
            raise ValueError(f"the essay is missing '{key}' — it is what makes it a letter to his students")
    letter = essay.get("letter") or {}
    if not (letter.get("question") and letter.get("answer")):
        raise ValueError("the essay is missing the student's letter")
